...
def connect_main_server() -> None:
    ...
    try:
        ...
        SelftosUtils.printf("<SERVER> [red]Connection refused by the main server.[/red]")
        exit(1)
...
def shutdown(users = users_list, reason: str = "No reason given.") -> None:
    ...
    """
    global is_running
    SelftosCommands.broadcast(users, f"Shutting down... [yellow]({reason})[/yellow]", render_on_server=True)
    for user in users:
        user.disconnect()
    is_running = False
    input_loop.stop()
...
def package_handler(package: SelftosNetwork.Package, sender: socket.socket) -> None:
    ...
    if package.type == "SFSUserData":
        ...
        SelftosCommands.broadcast(users_list, f"[cyan]{user.name}[/cyan] has joined the room.", render_on_server=True, exclude=user)
        ...
        if user.is_muted:
            ...
            return
        SelftosCommands.broadcast(users_list, f"{escape(str(package.content))}", msg_source=f"[cyan]{user.name}[/cyan]", render_on_server=True)
...
def client_handler(client: socket.socket, address: tuple) -> None:
    while is_running:
        ...
        if package is not None:
            ...
            if is_running:
                SelftosUtils.printf(f"<SERVER> Connection from {address} has been lost.")
                for user in users_list:
                    if user.client == client:
                        users_list.remove(user)
                        SelftosCommands.broadcast(users_list, f"[cyan]{user.name}[/cyan] has left the room.", render_on_server=True)
                        break
            break
...
def connection_handler() -> None:
    while is_running:
        try:
            ...
            SelftosUtils.printf(f"<SERVER> Connection from {address} has been established.")
    ...
    while is_running:  
        ...
        if command == "shutdown":
            ...
            if requested_command is None:
                SelftosUtils.printf(f"<SERVER> Command [red]'{command}'[/red] not found. Type 'help' to list commands.")
                continue
            else:
...
def start() -> None:
    ...
    #connect_main_server()
    try:
        SelftosUtils.printf("<SERVER> Starting the room...")
        ...
        SelftosUtils.printf(f"<[red]ERROR[/red]> [red]Can not start the server: {e}[/red]")
        exit(1)
