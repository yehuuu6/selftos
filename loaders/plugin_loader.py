...
class PluginLoader:
    PREFIX = f"<[magenta]PluginLoader[/magenta]>"
    ...
    def validate_plugin(self, plugin_instance, module_name: str) -> bool:
        ...
        for prop in required_properties:
            if not hasattr(plugin_instance, prop):
                SelftosUtils.printf(f"{self.PREFIX} [orange1]Warning:[/orange1] Missing required property [yellow]{prop}[/yellow] in [blue]{module_name}[/blue].")
                return False
        # Check if properties are callable
        for prop in required_properties[6:]:
            if not callable(getattr(plugin_instance, prop)):
                SelftosUtils.printf(f"{self.PREFIX} [orange1]Warning:[/orange1] Property [yellow]{prop}[/yellow] in [blue]{module_name}[/blue] is not callable.")
                return False
        return True
                ...
                try:
                    ...
                    if spec is not None:
                        ...
                        if not self.validate_plugin(plugin_instance, module_name):
                            SelftosUtils.printf(f"{self.PREFIX} [red]Error:[/red] Plugin [blue]{filename}[/blue] is not a valid plugin!")
                            continue
                        self.plugins.append(plugin_instance)
                        SelftosUtils.printf(f"{self.PREFIX} Loaded [blue]{plugin_instance.name}[/blue] successfully.")
                except Exception as e:
                    SelftosUtils.printf(f"{self.PREFIX} [red]Error:[/red] Failed to load [blue]{filename}[/blue]. Cause: {e}")
                    continue

        if len(self.plugins) == 0:
            SelftosUtils.printf(f"{self.PREFIX} No plugins were loaded.")
        else:
            SelftosUtils.printf(f"{self.PREFIX} [blue]{len(self.plugins)}[/blue] plugins are loaded and active. Type [italic]list [yellow]plugins[/yellow][/italic] to see them.")
    ...
    def unload_plugin(self, plugin_name: str) -> bool:
        SelftosUtils.printf(f"{self.PREFIX} [orange1]Warning:[/orange1] You may still require to remove the plugin from the plugin directory to completely unload it.")
        try:
            ...
            for plugin in self.plugins:
                ...
                if module_name == target_module_name:
                    ...
                    SelftosUtils.printf(f"{self.PREFIX} Unloaded [blue]{module_name}.pyd[/blue] successfully.")
                    return True
        except Exception as e:
            SelftosUtils.printf(f"{self.PREFIX} [red]Error:[/red] Failed to unload plugin. Cause: {e}")
            return False
        SelftosUtils.printf(f"{self.PREFIX} [red]Error:[/red] Plugin [blue]{target_module_name}[/blue] not found.")
        return False
